...
# 动作字典
actions = dict(up=0, down=1, left=2, right=3)
# 动作维度，也是神经网络要输出的维度
    ...
    def repay(self, batch_size):
        ...
        for i in batch_random_choice:
            ...
            if done:
                ...
                target = reward + self.alpha * (self.gamma * np.max(
                    self.model.predict(next_state)[0]) - target_f[0][action])
...
class Environ:
    ...
    def step(self, current_state, action):
        ...
        if next_state_pos[0] < 0 or next_state_pos[0] >= maze.shape[0] or next_state_pos[1] < 0 or next_state_pos[1] >= maze.shape[1] \
            ...
            next_state = matrix_to_img(target_state_pos[0],
                                       target_state_pos[1])
...
def predict():
    ...
    for i in range(100):
        ...
        # 在环境中施加行为推动游戏进行
        next_state, reward, done = environ.step(current_state, action)
        print('current_state: {}, action: {}, next_state: {}'.format(
            np.argwhere(current_state == TMP_VALUE)[0, 1:3],
            actions_new[action],
            np.argwhere(next_state == TMP_VALUE)[0, 1:3]))
        # 如果游戏结束，跳出循环
        if done:
            break
        # 使下一个状态成为下一帧的新状态
